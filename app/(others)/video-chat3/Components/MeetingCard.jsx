              ...
              &nbsp; Copy Link
				...
				{role === 'talent' && (
					<div className="flex justify-center items-center">
						...
						<Link
							href={`/consultants/${customData.consultant_real_id}`}
							target="_blank"
							rel="noopener noreferrer"
						>
							<span className="font-bold cursor-pointer">{customData.consultant_name}</span>
						</Link>
					</div>
				)}
				{role === 'consultant' && (
					<div className="flex justify-center items-center">
						...
						<span className="font-bold">{customData.talent_name}</span>
						{type === 'upcoming' && (
							<button
								disabled={downloading}
								onClick={() => handleDownload(talent_user_id)}
								className={`ml-2 text-white p-2 rounded ${
									downloading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500'
								}`}
							>
